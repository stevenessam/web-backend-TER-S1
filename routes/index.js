...
router.get('/searchDocumentsRelated/', (req, res) => {
    ...
    if (log.isInfoEnabled()) {
        log.info('searchDocumentsRelated - uri: [' + uri + ']');
    }

    if (uri.length === 0) {
        if (log.isInfoEnabled()) {
            log.info('searchDocumentsRelated - no parameter, returning empty response');
        }
        res.status(200).json({ result: [] });
    } else {
        ...
        uris.forEach(_uri => {
            // Insert the triple patterns into the SPARQL query
            let query = readTemplate("searchArticleRelated.sparql", _uri);
            if (log.isDebugEnabled()) {
                log.debug('searchDocumentsRelated - Will submit SPARQL query: \n' + query);
            }
            ...
            let _promise = (async () => {
                ...
                try {
                    _result = await d3.sparql(process.env.SEMANTIC_INDEX_SPARQL_ENDPOINT, query).then((data) => {
                        log.info('searchDocumentsRelated: query for uri ' + _uri + ' returned ' + data.length + ' results');
                        return data;
                    }).then(res => res);
                } catch (err) {
                    log.error('searchDocumentsRelated error: ' + err);
                    _result = err;
                }
                return _result;
            })();
        ...
        Promise.allSettled(promises).then((_promises) => {
            _promises.forEach((_promise, index) => {
                ...
                log.info("searchDocumentsRelated: current number of results : " + joinedResults.length);
            });
            log.info("searchDocumentsRelated: returning : " + joinedResults.length + " results");
            res.status(200).json({ "result": joinedResults });
        });
    }
});
